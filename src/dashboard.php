...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($titelErr) && empty($contentErr)) {
        ...
        $stmt->bind_param("ss", $titel, $content);

        if ($stmt->execute()) {
            echo "<script>alert('Nieuwsbericht succesvol toegevoegd!');</script>";
        } else {
        ...
        <section id="nieuwsbericht-form">
            ...
            <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                ...
                <div class="form-group">
                    ...
                    <textarea id="content" name="content" rows="4"><?php echo $content; ?></textarea>
                    <span class="error"><?php echo $contentErr; ?></span>
                </div>
                <button type="submit">Plaats Nieuwsbericht</button>
            </form>
        </section>
    </main>
